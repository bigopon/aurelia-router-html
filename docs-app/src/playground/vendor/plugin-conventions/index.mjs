...
function preprocessHtmlTemplate(unit, options, hasViewModel, fileExists = () => false) {
    var _a, _b;
...
function preprocessOptions(options = {}) {
    const { cssExtensions = [], jsExtensions = [], templateExtensions = [], useCSSModule = false, hmr = true, isDev = true, enableConventions = true, hmrModule = 'module', experimentalTemplateTypeCheck = false, ...others } = options;
    return {
...
function preprocess(unit, options, host) {
    ...
    if (allOptions.enableConventions && templateExtensions.includes(ext)) {
        for (const ce of allOptions.cssExtensions) {
            let filePair = `${fileName}.module${ce}`;
        ...
        return preprocessHtmlTemplate(unit, allOptions, allOptions.jsExtensions.some(e => host.fileExists(unit, `./${fileName}${e}`)), host.fileExists.bind(host));
    }
    if (allOptions.jsExtensions.includes(ext)) {
        for (const te of templateExtensions) {
            ...
            for (const te of templateExtensions) {
                ...
                if (!host.fileExists(unit, `./${viewPair}`))
                    continue;
